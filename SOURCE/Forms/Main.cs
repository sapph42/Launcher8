...
namespace Launcher8.Forms {
    public partial class Main : Form {
        ...
        private JsonSerializerOptions _options = new() {
            ...
            WriteIndented = true,
            Converters = { 
                new LauncherButtonJsonConverter(),
                new ButtonCollectionJsonConverter()
            }
        };
        #endregion
        #region ctor
        public Main(string version) {
            ...
            _buttons.AddRange(JsonSerializer.Deserialize<HashSet<LauncherButton>>(jsonString, _options));
        ...
        private void ButtonMouseDown(object? s, MouseEventArgs e) {
        ...
        private void ButtonMouseUp(object? s, MouseEventArgs e) {
            LauncherButton button = (LauncherButton)s!;
        ...
        private void EditToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            if (createOrEditButton.ReferenceType != LauncherButton.RefType.Undetermined) {
                ...
                if (createOrEditButton.KeyModifiers != Classes.ModifierKeys.None && createOrEditButton.HotkeyBase != Key.None) {
                    ...
                    button.KeyModifiers = Classes.ModifierKeys.None;
                    button.KeyTarget = Key.None;
                }
                button.ColorCheck();
                return;
            }
            ...
            if (createOrEditButton.Path is null)
                return;
            try {
            ...
            try {
                ...
                button.AdminOnly = createOrEditButton.AdminOnly;
                button.Background = createOrEditButton.Back;
                if (createOrEditButton.KeyModifiers != Classes.ModifierKeys.None) {
                    button.HasHotKeySet = true;
                    button.KeyModifiers = createOrEditButton.KeyModifiers;
                    button.KeyTarget = createOrEditButton.HotkeyBase;
                    button.HotKeyId = hook.RegisterHotKey(button.KeyModifiers, button.Keys);
                    ...
                    button.HasHotKeySet = false;
                    button.KeyModifiers = Classes.ModifierKeys.None;
                    button.KeyTarget = Classes.Key.None;
                }
                button.BackColor = button.Background;
            } catch {
                button.Caption = "";
        ...
        private void CopyJSONToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            if (!(strip?.SourceControl is LauncherButton button))
                return;
            string jsonString = JsonSerializer.Serialize(button, _options);
            Clipboard.Clear();
        ...
        private void RefreshCollection() {
            _buttons.Clear();
            var tempButtons = Controls.OfType<LauncherButton>().ToHashSet();
            var step1 = JsonSerializer.Serialize(tempButtons, _options);
            var step2 = JsonSerializer.Deserialize<HashSet<LauncherButton>>(step1, _options);
            _buttons.AddRange(step2);
            _buttons.AddRange(
                JsonSerializer.Deserialize<HashSet<LauncherButton>>(
                    JsonSerializer.Serialize(Controls.OfType<LauncherButton>(), _options),
                    _options
                )
            );
        }
